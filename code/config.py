...
class HyperParams:
    ...
    # ---- 激活/标准化 ----
    # activation: "relu" | "silu" | "gelu" | "prelu"
    activation: str = "silu"
...
PRESETS: Dict[str, Dict] = {
    ...
    # S3: 轻量快速（验证方向）
    # 仅在此 preset 内覆盖图扩散四项：K=2, alpha=0.5, diffusion_lambda=0.05, edge_drop_rate=0.20
    "S3": {
        "num_layers": 6,
        "dim": 64,
        "activation": "prelu",
        ...
        "pre_ln": True,
        "dropout": 0.3,
        "edge_drop_rate": 0.20,  # 覆盖为 0.20
        "appnp_K": 2,            # 覆盖为 2
        "appnp_alpha": 0.50,     # 覆盖为 0.50
        "lr": 5e-4,
        "weight_decay": 0.01,
        "focal_gamma": 0.0,
        "diffusion_lambda": 0.05 # 覆盖为 0.05
    },
}
...
# ========= Train config (data splits etc.) =========
@dataclass
class TrainConfig:
    paths: Paths = field(default_factory=Paths)
    # 默认用预设工厂生成超参（从 PRESET 读取）
