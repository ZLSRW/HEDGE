...
cv_5fold_ratio.py
------------------
在 CSV 图数据上，使用 XTalkRGCN 进行多种 train:val 比例的交叉验证，
用于评估模型的鲁棒性。

相对于原始 cv_5fold.py 的变化：

1) 不再只做一个固定的 5 折交叉验证，而是对如下比例分别重复实验：
   - zero_shot   : train_frac = 0.0  (0:10)
   - train1_val9 : train_frac = 0.1  (1:9)
   - train2_val8 : train_frac = 0.2  (2:8)
   - train3_val7 : train_frac = 0.3  (3:7)
   - train4_val6 : train_frac = 0.4  (4:6)
   - train5_val5 : train_frac = 0.5  (5:5)

   每种比例下仍然进行 --folds 次随机划分 (默认 5)，折名为 fold1..fold5。

2) 输出目录不再是单一的 out_dir，而是在同级新建 out_dir 的“+ratio 版本”，
   并在其下按比例创建子目录。例如：
   - 原: Results/
   - 现: Results_ratio/zero_shot/, Results_ratio/train1_val9/, ...

   若指定 --tag myrun，则：
   - 原: Results/myrun/
   - 现: Results/myrun_ratio/zero_shot/, Results/myrun_ratio/train1_val9/, ...

3) zero_shot 情况下，训练集为空；由于各损失函数对空 mask 已带兜底逻辑，
   不会报错。compute_metrics 也对“样本数为 0”增加了安全处理，返回全 NaN。
"""
...
def compute_metrics(y_true, y_prob, thr=0.5):
    """
    y_true / y_prob 为 numpy 数组。
    若 y_true 为空（例如 zero-shot 下训练集为空），直接返回全 NaN，避免 sklearn 报错。
    """
...
def save_interpret(fold_id, split, aux, mask, labels, out_dir, ids_map):
    ...
    gate_rows = np.array(
        [gates[r] if 0 <= r < Ng else [np.nan, np.nan, np.nan] for r in ranks]
    )  # (n_sel,3)
    ...
    df["gate_me"] = gate_rows[:, 0]
    df["gate_h"] = gate_rows[:, 1]
    df["gate_circ"] = gate_rows[:, 2]
    for r in range(R):
    ...
    with open(out_dir / f"fold{fold_id}_Astatic.json", "w") as f:
        json.dump(A_static.tolist(), f, indent=2)
...
def aux_heads_loss(aux, labels, train_mask, gene_mask, head_aux_lambda=0.2):
    ...
    idx_local = torch.tensor(
        [i for i in idx_local if i >= 0], dtype=torch.long, device=device
    )
...
def focal_bce_with_logits(logits, labels, mask, gamma=0.0, pos_weight=None):
    ...
    bce = torch.nn.functional.binary_cross_entropy_with_logits(
        x, y, pos_weight=pos_weight, reduction="none"
    )
    if gamma <= 0:
        return bce.mean()
    p = torch.sigmoid(x)
...
def relation_contrastive_loss(aux, labels, train_mask, gene_mask, tau=0.2):
    if not isinstance(aux, dict) or not all(
        k in aux for k in ["z_me", "z_h", "z_c"]
    ):
...
def diffusion_loss(aux, labels, gene_mask, train_mask, hps, device, model):
    ...
    sigma = torch.exp(
        torch.empty(Z.size(0), 1, device=device).uniform_(sigma_min, sigma_max).log()
    )
...
def irm_invariance_loss(
    model, data, labels, m_tr, gene_mask, xtalk, gextra, hps, device
):
...
def ns_logic_loss(aux, logits, labels, gene_mask, train_mask, hps, device, model):
    ...
    if sel.sum() < 2:
        return torch.tensor(0.0, device=device, requires_grad=True)
    gene_sel = sel[gene_mask]
    z_me = aux["z_me"][gene_sel]
    z_h = aux["z_h"][gene_sel]
    z_c = aux["z_c"][gene_sel]
    H = z_me.size(-1)
...
def subgraph_consistency_loss(
    aux, logits_base, model, data, labels, m_tr, gene_mask, xtalk, gextra, hps, device
):
    ...
    keep = mask_hard > 0.5
    ablate = {
        "me": not bool(keep[0].item()),
        "h": not bool(keep[1].item()),
        "circ": not bool(keep[2].item()),
    }
    out_sub = fwd_full(
        model,
        data.x,
...
def save_node_repr(fold_id, aux, labels, m_tr, m_va, gene_mask, out_dir, ids_map):
    ...
    def _save(split_name, split_mask_cpu):
        ...
        print(
            f"[info] 保存节点特征: {out_path} "
            f"(n={len(df)}, dim={feats.shape[1]})"
        )
...
def main():
    ...
    ap.add_argument(
        "--data_dir", type=str, default=".", help="Directory containing the CSVs"
    )
    ...
    ap.add_argument(
        ...
        type=str,
        default="Results",
        help="Output folder inside data_dir (base name)",
    )
    ap.add_argument(
        "--hps_json",
        type=str,
        default=None,
        help="Path to a JSON overriding HyperParams",
    )
    ap.add_argument(
        "--tag",
        type=str,
        default=None,
        help="Optional subfolder/tag under out_dir",
    )
    args = ap.parse_args()
    ...
    # ----- 原始结果目录 (仅用于保存一份默认 hparams，可保留兼容性) -----
    base_results_dir = Path(args.out_dir)
    if args.tag:
        base_results_dir = base_results_dir / args.tag
    base_results_dir.mkdir(parents=True, exist_ok=True)

    # ----- 本脚本实际写入的“ratio 根目录” -----
    # 规则：在 base_results_dir 的同级，新建一个 名称 + "_ratio" 的目录。
    # 例如: base_results_dir = "Results" -> ratio_root = "Results_ratio"
    #       base_results_dir = "Results/run1" -> ratio_root = "Results/run1_ratio"
    ratio_root = base_results_dir.parent / f"{base_results_dir.name}_ratio"
    ...
    # Save the hyperparams actually used for this run（保存一份在 base_results_dir 下）
    with open(base_results_dir / "hparams_used.json", "w") as _f:
        json.dump(
            {k: getattr(cfg.hparams, k) for k in vars(cfg.hparams)},
            _f,
            indent=2,
        )
    ...
    hps = cfg.hparams
    print(
        f"[HPS] focal={hps.focal_gamma}, rdrop={hps.rdrop_lambda}, "
        f"contrast=({hps.contrast_lambda},{hps.contrast_tau}), "
        f"diff={hps.diffusion_lambda}[{hps.diffusion_sigma_min},{hps.diffusion_sigma_max}], "
        f"irm={hps.irm_lambda}, ns={hps.ns_lambda}, "
        f"subgraph=({hps.subgraph_lambda}, k={hps.subgraph_k}, tau={hps.subgraph_tau}, "
        f"ent={hps.subgraph_entropy_lambda}), "
        f"smooth={hps.smooth_lambda}, head_aux={hps.head_aux_lambda}, "
        f"dropout={hps.dropout}, edge_drop={hps.edge_drop_rate}"
    )
    ...
    unify2orig = {
        int(u): int(o)
        for u, o in zip(
            loaded.node_ids["gene_unified_ids"], loaded.node_ids["gene_index"]
        )
    }
    ...
    print(f"[info] 有标签的基因节点数 = {len(labeled_gene_ids)}")
    ...
    ratio_settings = [
        ("zero_shot", 0.0),
        ("train1_val9", 1.0 / 10.0),
        ("train2_val8", 2.0 / 10.0),
        ("train3_val7", 3.0 / 10.0),
        ("train4_val6", 4.0 / 10.0),
        ("train5_val5", 5.0 / 10.0),
    ]

    for ratio_name, train_frac in ratio_settings:
        # 为当前比例创建独立的结果目录
        ratio_dir = ratio_root / ratio_name
        ratio_dir.mkdir(parents=True, exist_ok=True)

        print(
            f"\n================ 比例: {ratio_name} "
            f"(train_frac={train_frac:.2f}, val_frac={1.0 - train_frac:.2f}) "
            "================"
        )

        # 为当前比例保存一份 hparams（附加比例信息）
        hparams_with_ratio = {k: getattr(hps, k) for k in vars(hps)}
        hparams_with_ratio["ratio_name"] = ratio_name
        hparams_with_ratio["train_frac"] = float(train_frac)
        with open(ratio_dir / "hparams_used.json", "w") as _f:
            json.dump(hparams_with_ratio, _f, indent=2)

        # ---- 构造当前比例下的多次随机划分 ----
        n_lab = len(labeled_gene_ids)
        all_idx = np.arange(n_lab)
        splits = []

        if train_frac <= 0.0:
            # zero-shot: 无训练集，所有有标签基因都用于验证
            print(
                "[ratio] zero-shot 模式: 训练集为空, "
                "所有 labeled_gene_ids 仅作为验证集"
            )
            ...
            if len(np.unique(y_lab)) < 2:
                ...
                for tr_idx, va_idx in splitter.split(all_idx):
                    splits.append((tr_idx, va_idx))
            else:
                ...
                for tr_idx, va_idx in splitter.split(all_idx, y_lab):
                    splits.append((tr_idx, va_idx))

        rows = []
        fold_id = 0

        # ---- 对当前比例的每一个随机划分做一套完整训练/验证 ----
        for tr_idx, va_idx in splits:
            fold_id += 1
            print(
                f"\n[ratio={ratio_name}] [Fold {fold_id}/{args.folds}] "
                "---------------------"
            )
            ...
            try:
                ...
                model = XTalkRGCN(
                    ...
                    num_bases=hps.basis,
                    dropout=hps.dropout,
                    use_xtalk_gate=hps.enable_xtalk_gate,
                ).to(device)
            ...
            sch = torch.optim.lr_scheduler.CosineAnnealingLR(
                opt, T_max=hps.epochs, eta_min=hps.lr / 50.0
            )

            m_tr = torch.zeros_like(gene_mask)
            m_tr[tr_ids] = True
            m_va = torch.zeros_like(gene_mask)
            m_va[va_ids] = True

            print(
                f"[ratio={ratio_name}] fold={fold_id}: "
                f"n_train={int(m_tr.sum().item())}, "
                f"n_val={int(m_va.sum().item())}"
            )
            ...
            for ep in range(1, hps.epochs + 1):
                ...
                sup_loss = focal_bce_with_logits(
                    logits1, labels, m_tr, gamma=gamma, pos_weight=posw
                )
                ...
                if isinstance(aux1, dict):
                    ...
                    loss = loss + aux_heads_loss(
                        aux1, labels, m_tr, gene_mask, head_aux_lambda=lam_h
                    )
                ...
                if lam_c > 0 and isinstance(aux1, dict):
                    loss = loss + lam_c * relation_contrastive_loss(
                        aux1, labels, m_tr, gene_mask, tau=tau_c
                    )

                # structural modular losses (optional)
                loss = loss + diffusion_loss(
                    aux1, labels, gene_mask, m_tr, hps, device, model
                )
                loss = loss + irm_invariance_loss(
                    model, data, labels, m_tr, gene_mask, xtalk, gextra, hps, device
                )
                loss = loss + ns_logic_loss(
                    aux1, logits1, labels, gene_mask, m_tr, hps, device, model
                )
                loss = loss + subgraph_consistency_loss(
                    aux1,
                    logits1,
                    model,
                    data,
                ...
                if lam_r > 0:
                    loss = loss + lam_r * rdrop_kl(
                        logits1, logits2, train_gene_mask
                    )
                ...
                with torch.no_grad():
                    ...
                    if mv.sum() > 0:
                        y_true = (
                            labels[mv].detach().cpu().numpy().astype(int)
                        )
                ...
                print(
                    ...
                    f"val_AUPR={cur:.4f} (best={best['pr']:.4f})"
                )
                ...
                if bad >= 10:
                    print(
                        f"[EarlyStop] ratio={ratio_name}, fold={fold_id}, "
                        f"ep={ep}, best_val_aupr={best['pr']:.4f}"
                    )
                    break
            ...
            metrics_train = eval_and_dump(
                "train", logits, labels, m_tr, fold_id, ratio_dir, unify2orig
            )
            metrics_val = eval_and_dump(
                "val", logits, labels, m_va, fold_id, ratio_dir, unify2orig
            )
            ...
            if isinstance(aux, dict) and "rel_attn" in aux:
                save_interpret(
                    fold_id, "val", aux, m_va, labels, ratio_dir, unify2orig
                )

            # ★ 保存该折训练集 / 验证集的最终节点特征
            save_node_repr(
                fold_id, aux, labels, m_tr, m_va, gene_mask, ratio_dir, unify2orig
            )
            ...
            with torch.no_grad():
                ...
                df_ab = pd.DataFrame(
                    {
                        "gene_unified_id": idx,
                        "gene_index": [
                            int(unify2orig.get(int(i), -1)) for i in idx
                        ],
                        "prob_base": base[(labels >= 0) & m_va]
                        .detach()
                        .cpu()
                        .numpy(),
                        "prob_no_me": p_me0[(labels >= 0) & m_va]
                        .detach()
                        .cpu()
                        .numpy(),
                        "prob_no_h": p_h0[(labels >= 0) & m_va]
                        .detach()
                        .cpu()
                        .numpy(),
                        "prob_no_c": p_c0[(labels >= 0) & m_va]
                        .detach()
                        .cpu()
                        .numpy(),
                    }
                ...
                for col in ["me", "h", "c"]:
                    df_ab[f"delta_{col}"] = (
                        df_ab["prob_base"] - df_ab[f"prob_no_{col}"]
                    )
                df_ab.to_csv(
                    ratio_dir / f"fold{fold_id}_val_ablation.csv", index=False
                )
        ...
        df = df[order]
        df.to_csv(ratio_dir / "cv5_metrics_per_fold.csv", index=False)

        def agg(group):
            g = group.drop(columns=["fold", "split"], errors="ignore")
            return pd.Series(
                {
                    **{f"mean_{c}": g[c].mean() for c in g.columns},
                    **{f"std_{c}": g[c].std() for c in g.columns},
                }
            )

        summary = df.groupby("split").apply(agg).reset_index()
        summary.to_csv(ratio_dir / "cv5_metrics_summary.csv", index=False)

        print(
            f"[Done] ratio={ratio_name} 所有折完成, 结果已保存到: {ratio_dir}"
        )

    print("\n[Done] 所有比例均已完成, 根目录:", ratio_root)
